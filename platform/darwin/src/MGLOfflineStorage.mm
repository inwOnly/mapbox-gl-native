...
- (void)mergeOfflineRegions:(NSString *)databaseURL withCompletionHandler:(MGLOfflineMergeCompletionHandler)completion {
    self.mbglFileSource->mergeOfflineRegions(std::string(static_cast<const char *>([databaseURL UTF8String])), [&, completion](mbgl::expected<mbgl::OfflineRegions, std::exception_ptr> result) {
        NSError *error;
